...
function book_chip_value(array $book): string {
  ...
  if (isset($book['rating']) && $book['rating'] !== null) return number_format((float)$book['rating'], 2, ',', '');
  if (isset($book['release_year']) && $book['release_year'] !== null) return (string)(int)$book['release_year'];
  return '—';
}
function author_line(array $book): string {
  $author = $book['author_name'] ?? '';
  $extra  = $book['genre_name'] ?? '';
  $parts = array_filter([$author, $extra]);
  $html = htmlspecialchars(implode("<br>", $parts), ENT_QUOTES);
  // так как выше используется implode("<br>"), заменим безопасно на HTML перенос
